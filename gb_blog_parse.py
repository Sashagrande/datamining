...
class GbBlogParse:
    ...
    def parse_feed(self, url, soup):
        ...
        pag_urls = set(urljoin(url, pag_a.attrs['href']) for pag_a in pag_ul.find_all('a') if pag_a.attrs.get('href'))

        for pag_url in pag_urls:
            self.tasks.append(self.get_task(pag_url, self.parse_feed))

        posts = set(urljoin(url, post_a.attrs['href'])
    ...
    def parse_post(self, url, soup):
        data = {
            ...
            'tags_data': [{'url': urljoin(url, tag.attrs.get('href')), "name": tag.text}
